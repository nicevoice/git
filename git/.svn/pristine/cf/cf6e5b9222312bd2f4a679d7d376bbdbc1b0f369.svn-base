	...
	tinymce.create('tinymce.plugins.MediaPlugin', {
		init : function(ed, url) {
			...
			function isMediaElm(n) {
				return /^(ctvideo|mceItemFlash|mceItemShockWave|mceItemWindowsMedia|mceItemQuickTime|mceItemRealMedia)$/.test(n.className);
			}

            function replaceScriptWithCTVideoImage(content) {
                return typeof(content) == 'undefined' ? content : content.replace(/<(?:mce:)?script[^>]*data-role="ctvideo"[^>]*>\s*<\/(?:mce:)?script>/gim, function(script) {
                    script = '<script type="text/javascript" data-role="ctvideo" src="' + ed.dom.decode(getAttr(script, 'src')) + '"></script>';
                    return '<p style="text-align: center;" data-mce-style="text-align: center;"><img class="ctvideo" width="400" height="300" title="' + ed.dom.encode(script) + '" src="' + t.url + '/img/trans.gif" /></p>';
                });
            }
			...
			ed.addCommand('mceMedia', function() {
				...
				ed.windowManager.open({
					file : '?app=editor&controller=video&action=index',
					width : 450 + parseInt(ed.getLang('media.delta_width', 0)),
					height : 445 + parseInt(ed.getLang('media.delta_height', 0)),
					inline : 1
				}, {
					plugin_url : url,
					catid : params
			...
			ed.onInit.add(function() {
				...
				ed.selection.onSetContent.add(function(_ed, o) {
                    o.content = replaceScriptWithCTVideoImage(o.content);
			...
			ed.onPreProcess.add(function(ed, o) {
				...
				if (o.set) {
					...
					each(dom.select('IMG', o.node), function(n) {
						...
						if (isMediaElm(n)) {
							p = t._parse(n.title);
							dom.setAttrib(n, 'width', dom.getAttrib(n, 'width', p && p.width || 100));
							dom.setAttrib(n, 'height', dom.getAttrib(n, 'height', p && p.height || 100));
						}
					});
				}
			...
			ed.onPostProcess.add(function(ed, o) {
                ...
                o.content = o.content.replace(/<img[^>]+>/g, function(im) {
                    var cl = getAttr(im, 'class');
                    if (/^ctvideo$/.test(cl)) {
                        im = ed.dom.decode(getAttr(im, 'title'));
                    }
                    return im;
                });
			});
