...
if (priv::aca('exam', 'exam', 'unpublish')){
?>
      <input type="button" name="unpublish" onclick="exam.unpublish();" value="撤稿" class="button_style_1"/>
<?php 
}
...
if (priv::aca('exam', 'exam', 'createhtml')){
?>
      <input type="button" name="createhtml" onclick="exam.createhtml();" value="生成" class="button_style_1"/>
<?php 
}
?>
	</p>
...
?>
    <li class="question"><a href="#exam.question">设计表单</a></li>
<?php 
}
if (priv::aca('exam', 'report', 'index')){
?>
    <li class="report"><a href="#exam.report">分析报告</a></li>
<?php 
}
...
if (priv::aca('exam', 'exam', 'unpublish')){
?>
    <li class="unpublish"><a href="#exam.unpublish">撤稿</a></li>
<?php 
}
if (priv::aca('exam', 'exam', 'createhtml')){
?>
    <li class="createhtml"><a href="#exam.createhtml">生成</a></li>
<?php 
}
if (priv::aca('exam', 'exam', 'move')){
?>
...
var row_template = '<tr id="row_{contentid}">';
	...
	row_template +='	<td><a href="javascript:exam.view({contentid})" style="color:{color}" tips="ID：{contentid}<br />Tags：{tags}<br />创建：{createdbyname}（{created}）<br />修改：{modifiedbyname}（{modified}）<br />审核：{checkedbyname}（{checked}）<br/>题目数：{questions} <br/>人数上限：{maxanswers}<br/>开始时间：{starttime} <br/>截止时间：{endtime}" class="title_list">{title}</a> {note} {lock}</td>';
