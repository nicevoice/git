...
function imgmap(config) {
	...
	this.waterMark  = '<!-- Created by CmsTop (http://www.CmsTop.Com) -->';
 ...
 *	@param	flag	标记修改操作. (fromcircle, frompoly, fromrect, preserve)
...
imgmap.prototype._normCoords = function(coords, shape, flag) {
	...
	this.log('normcoords: ' + coords + ' - ' + shape + ' - ' + flag);
...
imgmap.prototype.setMapHTML = function(map) {
	...
	//remove all areas
	this.removeAllAreas();
	this.log("setMapHTML areas"+this.areas);
	...
	this.mapid   = oMap.id;
	var newareas = oMap.getElementsByTagName('area'); //解析map area到newareas集合
	var shape, coords, href, alt, title, target, id, pagemapid,cntid;
	for (var i=0, le = newareas.length; i<le; i++) {
		...
		pagemapid = newareas[i].getAttribute('id');
		if (pagemapid) {
			this.areas[id].amapid = pagemapid;
		}
		cntid = newareas[i].getAttribute('cntid');
		if (cntid) {
			this.areas[id].acntid = cntid;
		}
...
imgmap.prototype.scaleAllAreas = function(scale) {
	...
	this.log('gscale: '+this.globalscale);
	this.log('scale: '+scale);
	this.log('rscale: '+rscale);
...
function imgmap_spawnObjects(config) {
	this.log('spawnobjects');
	var maps = document.getElementsByTagName('map');
	var imgs = document.getElementsByTagName('img');
	var imaps = [];
	var imapn;
	this.log(maps.length);
