...
<div id="dms_content" class="dms_content">
	<div class="dms_inner">
		<div class="bk_8"></div>
		<form name="article_add" id="article_add" method="POST" action="?app=dms&controller=article&action=edit&id=<?=$id?>" onsubmit="return false;">
			...
			<table width="98%" border="0" id="tabledata" cellspacing="0" cellpadding="0"   class="table_form mar_l_8">
				...
				<tr>
					<th> Tags：</th>
					<td><input type="text" name="tags" value="<?=$tags?>" size="60" /></td>
				</tr>
				<tr>
					<th>来源：</th>
					<td class="c_077ac7">
				...
				<tr>
					...
					<td width="540" style="position:relative">
						<textarea name="content" id="content" style="visibility:hidden;height:350px;width:630px;"><?=$content?></textarea>
					</td>
					<td class="vtop"></td>
				</tr>
				<tr>
					<th>&nbsp;&nbsp;摘要：</th>
					<td width="540" style="position:relative">
						<textarea name="description" style="height:40px;width:621px; background:#fff;"><?=$description?></textarea>
					</td>
				</tr>
				<tr>
				...
				<?php foreach($expand as $key=>$item):?>
				<tr class="expandinput">
					<th></th>
					<td>
					<input type="text" onblur=" this.value || (this.value = '变量名')" onfocus="this.value == '变量名' && (this.value = '')" value="<?=$item->key?>" name="expand[<?=$key?>][key]">
					<input type="text" onblur=" this.value || (this.value = '值')" onfocus="this.value == '值' && (this.value = '')" value="<?=$item->value?>" name="expand[<?=$key?>][value]">
...
var templateIndex	= <?=count($expand);?>;

var expandRowAdd = function(obj) {
	var templateRow	= '<tr class="expandinput">\
<th width="60"></th><td><input type="text" name="expand[' + templateIndex + '][key]" value="变量名" onfocus="this.value == \'变量名\' && (this.value = \'\')" onblur=" this.value || (this.value = \'变量名\')" /><input type="text" name="expand[' + templateIndex + '][value]" value="值" onfocus="this.value == \'值\' && (this.value = \'\')" onblur=" this.value || (this.value = \'值\')" /><span class="remove" onclick="removeField(this);"></span></td>\
</tr>';
