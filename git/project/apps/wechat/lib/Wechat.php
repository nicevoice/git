...
class Wechat {
    ...
    public function reply($msg = array(), $return = false) {
        ...
        //debug 调试记录回复信息
        if ($this->debug) {
            file_put_contents($this->debugpath . "./reply.txt", "\n---" . date('Y-m-d H:i:s') . "\n" . $xmldata, FILE_APPEND);
        }
    }

    function login2() { //初始化，登陆微信平台
        global $cookie_file;
        $url = "http://mp.weixin.qq.com/cgi-bin/login?lang=zh_CN";
        $fields = array('username' => $this->wechatOptions['account'], 'pwd' => md5($this->wechatOptions['password']), 'imgcode' => '', 'verify' => '', 'f' => 'json');
...
// 			return $result['ErrCode'];
    ...
    private function post($url, $postfields, $refer) {
        dump(array($url, $postfields, $refer));
        $ch = curl_init($url);
        $options = array(
