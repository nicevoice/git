...
?>
  ...
  <tr>
    ...
    <td><label><input type="radio" name="iscreateindex" value="1" class="radio_style" <?php if ($iscreateindex == 1) echo 'checked';?> /> 是</label>&nbsp;&nbsp;
        <label><input type="radio" name="iscreateindex" value="0" class="radio_style" <?php if ($iscreateindex == 0) echo 'checked';?>> 否</label>&nbsp;&nbsp;&nbsp;&nbsp;<span class="c_gray">如果通过页面功能管理栏目首页，则应选择否。</span>
    </td>
  </tr>
  <tr>
    <th>栏目首页URL规则：</th>
  ...
  <tr>
    <th>列表页每页信息数：</th>
    <td>&nbsp;<input type="text" name="pagesize" value="<?=$pagesize;?>" size="5"/> <span class="c_gray"> 每个栏目可独立设置，留空则取 <a href="javascript:;" onclick="ct.assoc && ct.assoc.open('?app=system&controller=setting&action=optimize', '_blank');">系统设置</a> 中的值</span></td>
  </tr>
  <tr>
    <th>允许用户投稿：</th>
  ...
  <tr>
  	<th>关键词：</th>
  	<td>&nbsp;<input type="text" name="keywords" value="<?=$keywords?>" size="60" maxlength="255"/></td>
  </tr>
  <tr>
  	<th>描述：</th>
