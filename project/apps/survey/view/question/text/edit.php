<form id="text_edit" method="post" action="?app=<?=$app?>&controller=<?=$controller?>&action=<?=$action?>">
	...
	<table border="0" cellspacing="0" cellpadding="0" class="table_form">
	  ...
	  <tr>
		<th>数据校验：</th>
		<td><input type="radio" name="validator" value="" checked="checked" class="bdr_5" <?=$validator ? '' : 'checked'?>/> 无 <input type="radio" name="validator" value="date" class="bdr_5" <?=$validator=='date' ? 'checked' : ''?>/> 日期 <input type="radio" name="validator" value="email" class="bdr_5" <?=$validator=='email' ? 'checked' : ''?>/> E-mail <input type="radio" name="validator" value="number" class="bdr_5" <?=$validator=='number' ? 'checked' : ''?>/> 数字</td>
	  </tr>
	  <tr>
		<th>必填：</th>
