 ...
 * @aca 问卷报告
 */
final class controller_admin_report extends survey_controller_abstract {

    private $survey, $question, $answer, $answer_record, $answer_option, $pagesize = 10;
    ...
    function index() {
        ...
        if (!$survey)
            $this->showmessage('调查不存在');

        $this->priv_category($survey['catid']);

        $this->view->assign($survey);
        $this->view->assign('head', array('title' => '分析报告：' . $survey['title']));
    ...
    function question() {
        if (isset($_GET['report']) && $_GET['report']) {
            ...
            if (in_array($question['type'], array('radio', 'checkbox', 'select'))) {
                ...
                $this->view->assign('report_name', 'survey');
            ...
            $questionid = isset($_GET['questionid']) ? $_GET['questionid'] : (isset($survey['question'][0]['questionid']) ? $survey['question'][0]['questionid'] : 0);
            $this->view->assign('questionid', $questionid);
            $this->view->assign($survey);
            $this->view->assign('head', array('title' => '分项报表：' . $survey['title']));
    ...
    function answer() {
        if (isset($_GET['report']) && $_GET['report']) {
            ...
            $this->priv_category($survey['catid']);
            $this->view->assign($survey);
            $this->view->assign('head', array('title' => '全部数据：' . $survey['title']));
    ...
    function option() {
        ...
        if (isset($_GET['report']) && $_GET['report']) {
            ...
            $this->view->assign('head', array('title' => '调查记录：' . $name));
    ...
    function view() {
        ...
        $this->view->assign('head', array('title' => '查看问卷：' . $survey['title']));
    ...
    function search() {
        ...
        if ($this->is_post()) {
            ...
            $this->view->assign($survey);
            $this->view->assign('head', array('title' => '高级检索：' . $survey['title']));
