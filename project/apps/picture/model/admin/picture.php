<?php
class model_admin_picture extends model implements SplSubject
{
	...
	function __construct()
	{
		...
		$this->_fields = array('contentid', 'editor', 'description', 'total');
        ...
        $this->modelid = modelid('picture');
	}
	
	public function __call($method, $args)
	{
		if(!priv::aca('picture', 'picture', $method)) return true;
	...
	function get($contentid, $fields = '*', $action = null, $table_picture = true)
	{
		...
		$this->fields = $fields;
		$this->action = $action;
		$this->table_picture = $table_picture;

		$this->event = 'before_get';
		$this->notify();
		
		if ($this->table_picture)
		{
			$this->data = $this->db->get("SELECT $this->fields FROM `#table_content`, `#table_picture` WHERE `#table_content`.`contentid`=`#table_picture`.`contentid` AND `#table_content`.`contentid`=$this->contentid");
	...
	function ls($where = null, $fields = '*', $order = 'c.`contentid` DESC', $page = null, $pagesize = null, $table_picture = false)
	{
		$this->where = $where;
		$this->fields = $fields;
		$this->order = $order;
		...
		if ($this->table_picture)
		{
			...
			$this->sql = "SELECT $this->fields FROM `#table_content` c, `#table_picture` p WHERE c.`contentid`=p.`contentid`";
			...
			if ($this->data)
			{
			    ...
			    if (!is_null($page))
			    {
			    	$sql = "SELECT count(*) as `count` FROM `#table_content` c, `#table_picture` p WHERE c.`contentid`=p.`contentid`";
	...
	function add($data)
	{
		...
		$this->data['total'] = count($this->data['pictures']);
		$data = $this->filter_array($this->data, array('contentid', 'editor', 'description', 'total'));
