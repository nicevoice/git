...
class model_admin_export extends model 
{
	...
	function getanswers($contentid){
		$r= $this->db->select("SELECT * FROM `#table_exam_answer` WHERE `contentid` = $contentid order by created desc");
		return $r;
	}
	function getrecodes($answerid)
	{
 
		$r = $this->db->select("SELECT * FROM `#table_exam_answer_record` WHERE `answerid` = $answerid");
		return $r;
	}
	
	function getoptions($answerid)
	{
		$r = $this->db->select("SELECT * FROM `#table_exam_answer_option` WHERE `answerid` = $answerid ");
		return $r;
	}
