...
<div>
	<div class="f_l w_80 tag_list_2" style="height:600px;">
		<ul>
			...
			<li><a href="?app=exam&controller=report&action=question&contentid=<?=$contentid?>" class="subentry_report_title s_6">分项报表</a></li>
			<li><a href="?app=exam&controller=report&action=answer&contentid=<?=$contentid?>" class="all_data_title">全部数据</a></li>
		</ul>
	</div>
...
?>
	<li class="question_list_270 <?=$b?>"><a href="?app=exam&controller=report&action=question&contentid=<?=$contentid?>&questionid=<?=$q['questionid']?>"><?=$i?>. <?=$q['subject']?></a></li>
<?php
	$i++;
}
?>
...
<div class="tag_1" ><a style="margin:3px 0px 0px 5px" class="f_l" href="javascript:;" onclick="exam.exportdata(<?=$contentid?>,<?=$questionid?>)">导出此项</a>
 </div>
...
var url = '<?=ADMIN_URL?>?app=exam&controller=report&action=question&questionid=<?=$questionid?>&report=1';
...
{
?>
<div class="tag_1" style="padding-top:2px;"><a class="f_l" href="javascript:;" onclick="exam.exportdata(<?=$contentid?>,<?=$questionid?>)">导出此项</a>
...
var tableApp = new ct.table('#item_list', {
    ...
    baseUrl  : '?app=exam&controller=report&action=question&questionid=<?=$questionid?>&report=1'
});
